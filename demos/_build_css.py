...
.brand__mark svg {{ width: 24px; height: 24px; display: block; color: var(--on-accent); }}
.brand__logo {{ height: 46px; width: auto; max-width: 240px; display: block; object-fit: contain; }}
.brand__logo--stack, .brand__logo--badge {{ height: 58px; }}
@media (max-width: 480px) {{
  .brand__logo {{ height: 38px; max-width: 170px; }}
  .brand__logo--stack, .brand__logo--badge {{ height: 50px; }}
}}
.foot .brand {{ margin-bottom: .2rem; }}
.foot__tile {{ display: inline-grid; place-items: center; background: #fff; border-radius: 10px; padding: .45rem .7rem; }}
.foot .brand__logo {{ height: 34px; max-width: 200px; }}
.foot .brand__logo--stack, .foot .brand__logo--badge {{ height: 46px; }}
.foot .brand__mark {{ width: 34px; height: 34px; }}
.foot .brand__mark svg {{ width: 20px; height: 20px; }}
...
.brand__sub {{ display: block; font-family: var(--font-body); font-size: .68rem; font-weight: 500; letter-spacing: .13em; text-transform: uppercase; color: var(--ink); }}
...
/* overlay hero: full-bleed photograph, optional video on top, tint above both */
.hero--overlay {{ background: var(--ink); color: #fff; overflow: hidden; }}
...
.hero--overlay .hero__inner {{ z-index: 3; }}
@media (prefers-reduced-motion: reduce) {{
  .hero--overlay .hero__bg {{ animation: none; }}
...
/* ---------- marquee band ---------- */
.band {{ background: var(--cta); color: var(--on-accent); padding-block: .9rem; overflow: hidden; }}
...
.band__track span {{
  ...
  font-family: var(--font-display); font-weight: 700; font-size: clamp(.9rem, 1.5vw, 1.05rem);
  letter-spacing: .06em; text-transform: uppercase; white-space: nowrap;
}}
.band__track span::after {{ content: ""; width: 8px; height: 8px; background: currentColor; opacity: .7; transform: rotate(45deg); flex: none; }}
...
.gallery .shot {{ aspect-ratio: 4 / 3; transition: transform .6s var(--ease), box-shadow .6s var(--ease); }}
.gallery .shot:hover {{ transform: translateY(-6px) scale(1.015); box-shadow: var(--shadow-lg); }}
.shot--hero {{ background-image: var(--hero-img); }}
...
.foot .brand__sub {{ color: rgba(255,255,255,.82); }}
...
.foot__rocket {{ width: 1.25em; height: 1.25em; vertical-align: -.3em; margin-right: .3em; color: #fff; }}
